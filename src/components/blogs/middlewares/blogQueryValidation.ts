...
const createQueryPaginationChain = () =>
  query(["pageNumber", "pageSize"]).optional().isNumeric().withMessage("Can't be a string");
// .trim

const createQuerySortByChain = () =>
  query("sortBy")
    .optional()
    .custom((value) => {
      if (!isNaN(value)) {
...
const createQuerySortDirectionChain = () =>
  query("sortDirection")
    .optional()
    .isIn(["asc", "desc"])
